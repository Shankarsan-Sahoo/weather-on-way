...
# -------------------------------
# 🔑 Use keys from .env via settings
# -------------------------------
GOOGLE_API_KEY = settings.GOOGLE_API_KEY
OPENWEATHER_KEY = settings.OWM_API_KEY

# -------------------------------
# 1. Haversine for distance
# -------------------------------
def haversine(a, b):
    lat1, lon1 = a
    lat2, lon2 = b
...
def get_weather(lat, lon, max_retries=3):
    ...
    # Return statement moved inside try block

# -------------------------------
# 6. Main route forecast (returns list)
# -------------------------------
def route_forecast_cl(origin, destination, step_km=10, start_time="08:00"):
...
class HomeView(View):
    ...
    def post(self, request):
        ...
        try:
            route_data, total_distance = route_forecast_cl(
                origin, 
                destination, 
                step_km=20,  # Larger steps for better performance
                start_time=departure_time.split('T')[1][:5]  # Extract HH:MM from datetime-local
            )
...
# This function is no longer used as we're using the HomeView class-based view
